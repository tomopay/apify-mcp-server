...
| **Dev server (no UI)** | `http://localhost:3001/mcp` / tools: `mcp__apify-dev__*`            | Test tools via direct MCP connection (local sessions only)  |
| **Dev server (UI)**    | `http://localhost:3001/mcp?ui=true` / tools: `mcp__apify-dev-ui__*` | Test tools with widget rendering (local sessions only)      |
...
### mcpc probing
- [ ] `npm run build && mcpc @stdio restart`
- [ ] Call the new/changed tools and confirm output matches the spec
- [ ] Test edge cases: invalid input, non-existent resources, error messages
...
- [ ] mcpc probe confirms behavior matches spec
